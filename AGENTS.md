...
- Requires Python 3.8+ with PyTorch 2.0+
- Install dependencies: `pip install torch numpy matplotlib scipy tqdm asciichartpy`
- Optional: `pip install idefix-pytools` (for data conversion scripts)
- System: `ffmpeg` (for MP4 movie generation in visualization scripts)
- GPU required (CUDA, no CPU fallback)

### Training

```bash
python src/train.py --param <parameter_name> [--experiments-dir <path>] [--fast] [--batch-size N] [--patience N] [--resume]
```

- **Parameters:** `density`, `vy`, `by`, etc. (defaults to `density`; any string accepted)
- **Epochs:** 5,000
- **Learning rate:** 0.001 with StepLR scheduler (step=500, gamma=0.5)
- **Batch size:** 16 (default, override with `--batch-size`)
- **Loss:** MAE (`F.l1_loss`) on normalized targets
- **Early stopping:** `--patience N` (default 500 epochs; 0 disables)
- **Resume:** `--resume` restores from `training_state.pt`; requires loss version `'normalized-mae-only-v1'`
- **Mixed precision:** bf16 if supported, fp16 otherwise (SpectralConv3d always uses float32)
- **Validation:** uses `data/<param>/val/` for model selection and early stopping; `test/` is never read during training
- `--fast` runs a smoke test (3 epochs, batch_size=4, 1 file) with frequent visualizations
- **Outputs:**
  ...
  - `model_best.pt` saved whenever validation MAE improves; `training_state.pt` for full resume state
- Checkpoints created before the MAE-only refactor are intentionally incompatible with `--resume`

### Inference

```bash
python src/inference.py --param <parameter_name> [--experiments-dir <path>] [--rollout-steps N] [--checkpoint <path>]
```

- Resolves model artifacts from `--experiments-dir` and `--param`; prefers `model_best.pt`, falls back to `model_64_30.pt`
...
```bash
python src/viz_scalar.py --param <parameter_name> [--experiments-dir <path>] [--force] [--max-frames N] [--workers N]
python src/viz_vector.py --param <parameter_name> [--experiments-dir <path>] [--force] [--max-frames N] [--workers N] [--quiver-stride N]
```

- `viz_scalar.py` loads `pred_sim_*.npy` files and renders scalar diagnostics (field maps, power spectra, ε_M) without re-running the model; uses `ref_sim_<id>.npy` as reference if present
- `viz_vector.py` loads paired component predictions (e.g. `bx` + `by` or `vx` + `vy`) and renders magnitude colormaps with quiver arrows
- `--force`: regenerate existing renders; `--max-frames N`: limit frames per sim; `--workers N`: parallel rendering
- `--quiver-stride N` (default 8, viz_vector.py only): quiver arrow subsample stride
- Both scripts generate MP4 movies via `ffmpeg` after frame rendering
...
- **Training:** `data/<param>/train/x_sim_<id>.npy` / `y_sim_<id>.npy` (count determined dynamically)
- **Validation:** `data/<param>/val/x_sim_<id>.npy` / `y_sim_<id>.npy` (count determined dynamically)
- **Test:** `data/<param>/test/x_sim_<id>.npy` / `y_sim_<id>.npy` (count determined dynamically; used only for final evaluation)
...
4. `python convert_to_npy.py [--runs-dir runs] [--output-dir ../../data]` → `data/<param>/[train|test]/[x|y]_<idx>.npy`
   ...
   - 20 evenly-spaced sliding windows per sim (stride 51 frames); input: 5 consecutive frames; output: 20 consecutive frames immediately after input
   - nu and mu appended as channels 5–6; files named `x_sim_<id>.npy` / `y_sim_<id>.npy`
...
- Input frames (first 5 channels): min-max scaled to [-1, 1] per file at load time
...
- **Internal:** `get_grid()` appends 3 coordinate channels -> 10 channels, lifted to width 30; grid cached via `register_buffer`
...
**SpectralConv3d:** 3D real FFT -> complex multiplication (4 weight tensors for x/y octants) -> inverse FFT; always executes in float32 regardless of autocast

**File locations:**
- Model definitions: `src/architecture.py` -- single canonical definition, imported by both `train.py` and `inference.py`
...
- `src/architecture.py` -- model definitions (FNO3d, SpectralConv3d)
- `src/train.py` -- training logic
- `src/inference.py` -- inference logic
- `src/viz_scalar.py` -- scalar prediction visualization and MP4 movie generation
- `src/viz_vector.py` -- vector prediction visualization and MP4 movie generation
- `src/experiment_layout.py` -- shared legacy/run-scoped experiment path resolver
- `src/prepare_reference.py` -- converts VTK output to dense `ref_sim_<id>.npy` reference trajectories
- `src/utilities.py` -- `LpLoss` (relative/absolute Lp norm loss)
- `src/Adam.py` -- custom Adam optimizer (not used by current training loop)
- `src/architecture_diagram.py` -- architecture diagram generation
- `src/unify_legacy_magnetic_run.py` -- one-off migration utility for legacy bx/by artifacts
- `data/idefix/render_vtk.py` -- standalone VTK snapshot renderer (independent of training pipeline)

## Known Issues

1. **Unused BatchNorm layers:** `bn0`-`bn3` are defined in `FNO3d.__init__` but never called in `forward()`.

2. **Preprocessing script:** `data/idefix/convert_to_npy.py` handles VTK → `.npy` conversion for Idefix output. For FARGO3D `.dat` files, an equivalent conversion script is not in this repository.

3. **Autoregressive error accumulation:** the `--rollout-steps N` flag in `inference.py` enables free-running rollout, and prediction errors compound at each chained step.
